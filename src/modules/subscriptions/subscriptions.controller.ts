...
export class SubscriptionsController {
  ...
  @Get('sub/:clientId')
  @ApiOperation({ summary: 'Получить подписку (VLESS-конфиги)', description: 'Возвращает base64-строку с VLESS-ссылками (по одной на строку). Для каждого пула выбирается наименее нагруженный сервер. Формат совместим с v2rayTun, Happ, Streisand.' })
  @ApiParam({ name: 'clientId', description: 'UUID клиента', example: 'a1b2c3d4-e5f6-7890-abcd-ef1234567890' })
  @ApiResponse({ status: 200, description: 'Base64 строка с VLESS-ссылками', type: String })
